import {Button,Divider,Form,Input,Modal,Select,Table,Tag,message,Spin,DatePicker,} from "antd";
import React, { useState, useEffect } from "react";
import { validateMessages } from "../../..//utils/requireMessage";
import dayjs from "dayjs";
import locale from "antd/lib/date-picker/locale/vi_VN";
import moment from "moment";
import {useAddTopicMutation,useDeleteTopicMutation,useEditTopicMutation,useGetTopicsQuery,} from "../../../redux/Slices/Topic";
...
export default function Index() {
  ...
  const [deleteTopicById, { isLoading: isLoadingDelete }] =
  ...
  return (
    <div>
      ...
      <Divider>
        <span style={{ fontSize: 28 }}>TABLE OF TOPICS & IDEAS</span>
      </Divider>
      ...
      >
        ...
        >
          ...
          <Form.Item
            label="Close Topic Date"
            name="closureDateTopic"
            rules={[{ required: true }]}
          >
            <DatePicker locale={locale} format={"DD MMM, YYYY"} />
          </Form.Item>
          <Form.Item
            label="Date Idea Range"
            name="dateIdea"
            rules={[{ required: true }]}
          >
...
const EditModal = ({ isOpenEdit, setIsOpenEdit, dataEdit, setDataEdit }) => {
  const { data: departments } = useGetDepartmentsQuery();
  const [updateTopicById, { isLoading }] = useEditTopicMutation();
  const [form] = Form.useForm();
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      dateIdea: [
        dayjs(dataEdit?.openDate, "YYYY-MM-DD"),
        dayjs(dataEdit?.closureDateIdea, "YYYY-MM-DD"),
      ],
    });
  }, [dataEdit?.id]);
  return (
